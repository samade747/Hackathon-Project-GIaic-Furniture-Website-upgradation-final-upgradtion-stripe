...
export default function Billing() {
  ...
  const handlePlaceOrder = async () => {
    try {
      // For all payment methods, let's build an order payload:
      ...
      // If user selected Card Payment, first create a Stripe Checkout Session
      if (paymentMethod === "Card Payment") {
        // 1) Make an API call to /api/create-checkout-session
        // We'll pass just the cart items, but you can pass more info as needed
        const stripeRes = await fetch("/api/create-checkout-session", {
          ...
          body: JSON.stringify({
            items: orderItems.map((item) => ({
              name: item.name,
              price: item.price, // Make sure it's in the currency your Stripe account uses
              quantity: item.quantity,
            })),
          }),
        });
        ...
        // 2) If successful, redirect user to Stripe Hosted Checkout
        if (data.url) {
          // Optionally, you could store the order in your DB with a "pending payment" status
          // For now, let's just redirect:
          window.location.href = data.url;
          return; // IMPORTANT: Return here so we don’t run the code below
        }
      }

      // If it's NOT card payment, we do the normal place-order logic here:
      ...
      console.log("Order created in DB:", dbData.order);
      clearCart();
      alert("Order placed successfully!");

      // e.g. router.push("/order-confirmation?orderId=" + dbData.order._id)
    } catch (err) {
